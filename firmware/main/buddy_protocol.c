...
static bool buddy_parse_token_monitor(const cJSON *object, buddy_token_monitor_t *tm)
{
    const cJSON *tm_obj = cJSON_GetObjectItemCaseSensitive(object, "token_monitor");
    const cJSON *tools_arr;
    const char *currency_str;
    size_t currency_len;
    unsigned i;

    memset(tm, 0, sizeof(*tm));
    if (tm_obj == NULL || !cJSON_IsObject(tm_obj)) {
        return true;
    }
    ...
    if (buddy_json_optional_string(tm_obj, "currency", &currency_str, &currency_len) && currency_str != NULL) {
        ...
        strcpy(tm->currency, "USD");
    }

    tools_arr = cJSON_GetObjectItemCaseSensitive(tm_obj, "tools");
    if (tools_arr != NULL && cJSON_IsArray(tools_arr)) {
        int count = cJSON_GetArraySize(tools_arr);
        if (count > BUDDY_TOKEN_TOOL_MAX) {
            count = BUDDY_TOKEN_TOOL_MAX;
        }
        tm->active_tools_count = (unsigned)count;
        for (i = 0; i < (unsigned)count; ++i) {
            const cJSON *item = cJSON_GetArrayItem(tools_arr, (int)i);
            if (item != NULL && cJSON_IsObject(item)) {
                const char *name_str;
                size_t name_len;
                unsigned cost = 0;
                unsigned used = 0;
                if (buddy_json_optional_string(item, "name", &name_str, &name_len) && name_str != NULL) {
                    buddy_copy_utf8(tm->tools[i].name, sizeof(tm->tools[i].name), name_str, name_len, NULL);
                }
                (void)buddy_json_u64(item, "tokens_today", &tm->tools[i].tokens_today);
                (void)buddy_json_unsigned(item, "cost_cents", &cost);
                (void)buddy_json_unsigned(item, "used_percent", &used);
                tm->tools[i].cost_cents = cost;
                tm->tools[i].used_percent = (uint8_t)(used > 100 ? 100 : used);
                tm->tools[i].available = true;
            }
        }
    }
...
static bool buddy_parse_heartbeat(const cJSON *object, buddy_event_t *event)
{
    ...
    if (!buddy_parse_token_monitor(object, &event->heartbeat.token_monitor)) {
        return false;
    }
    /* 如果未显式提供 token_monitor.tokens_today，回退使用心跳根节点的 tokens_today */
    if (event->heartbeat.token_monitor.tokens_today == 0 && event->heartbeat.tokens_today > 0) {
        event->heartbeat.token_monitor.tokens_today = event->heartbeat.tokens_today;
        event->heartbeat.token_monitor.available = true;
    }
